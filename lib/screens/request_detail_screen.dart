...
class _RequestDetailScreenState extends State<RequestDetailScreen> {
  ...
  Widget _buildSOCard(SOModel so) {
    ...
    final percentageColor = percentage > 30
        ? Colors.red
        : (percentage > 0 ? Colors.orange : AppTheme.accentColor);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            if (so.orderDate.isNotEmpty) _buildDetailRow('Order Date', so.orderDate, Icons.calendar_today),
            if (so.sapSoNumber.isNotEmpty) _buildDetailRow('SAP SO', so.sapSoNumber, Icons.receipt_long),
            if (so.sapPoNumber.isNotEmpty) _buildDetailRow('SAP PO', so.sapPoNumber, Icons.shopping_cart),
            if (so.line.isNotEmpty) _buildDetailRow('Line', so.line, Icons.sort),
            if (so.materialCode.isNotEmpty) _buildDetailRow('Material', so.materialCode, Icons.inventory),
            if (so.batchCardNo.isNotEmpty) _buildDetailRow('Batch Card', so.batchCardNo, Icons.badge),
            if (so.packDate.isNotEmpty) _buildDetailRow('Pack Date', so.packDate, Icons.event),
            if (so.source.isNotEmpty) _buildDetailRow('Source', so.source, Icons.source),
            const SizedBox(height: 8),
            Row(
              children: [
                Expanded(
