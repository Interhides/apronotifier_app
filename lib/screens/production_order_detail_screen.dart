...
class _ProductionOrderDetailScreenState
  ...
  Widget _buildPOCard(ProductionOrderModel po) {
    ...
    final percentage = ((po.sf - po.orderQty) / po.orderQty * 100);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            if (po.orderDate.isNotEmpty) _buildDetailRow('Order Date', po.orderDate, Icons.calendar_today),
            if (po.sapPoNumber.isNotEmpty) _buildDetailRow('SAP PO', po.sapPoNumber, Icons.shopping_cart),
            if (po.line.isNotEmpty) _buildDetailRow('Line', po.line, Icons.sort),
            if (po.materialCode.isNotEmpty) _buildDetailRow('Material', po.materialCode, Icons.inventory),
            if (po.batchCardNo.isNotEmpty) _buildDetailRow('Batch Card', po.batchCardNo, Icons.badge),
            if (po.packDate.isNotEmpty) _buildDetailRow('Pack Date', po.packDate, Icons.event),
            if (po.source.isNotEmpty) _buildDetailRow('Source', po.source, Icons.source),
            const SizedBox(height: 8),
            Row(
              children: [
                Expanded(
